...
def showft(tt, sig_xt, Fs, ff_lim):
    ...
    tlen = tt[-1]	
    xt = (0.999*sig_xt.signal()/float(max(abs(sig_xt.signal()))))
...
def showeye(xt, Fs, FB, NTd=50, dispparms=[]):
    ...
    for ind in np.arange(1,NTd):
        TM = np.vstack((TM, rt[trix[ind]:int(trix[ind]+dws)]))    # Second trace
    plt.figure()
...
def showpsd(sig_xt, Fs, ff_lim, N):
    ...
    rt = (0.999*sig_xt.signal()/float(max(abs(sig_xt.signal()))))                               # Get x(t) 
